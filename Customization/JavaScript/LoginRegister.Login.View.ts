...
import AccountLoginModel = require('../../../Advanced/Account/JavaScript/Account.Login.Model');
...
const LoginRegisterLoginView: any = BackboneView.extend({
    ...
    submitForm: function(e, model, props) {
        ...
        if($('#g-recaptcha-response').val()===""){
            ...
            const global_view_message = new GlobalViewsMessageView({
                message:Utils.translate('Please verify that you are not a robot.'),
                // message:message,
                type: 'error',
                closable: true,
            ...
            this.$('[data-id="alert-type"]').html(global_view_message.render().$el.html());
            return jQuery.Deferred().reject();
        }else {
            this.$('[data-id="alert-type"]').html("");
    ...
    refreshApplication: function(response) {
        ...
        if (
            ...
            response.touchpoints &&
                (application.Configuration.siteSettings.touchpoints = response.touchpoints);
    ...
    getContext: function() {
        ...
        return {
            ...
            isSkipLogin: !!(
                this.$containerModal && this.application.getConfig('checkoutApp.skipLogin')
            ),
