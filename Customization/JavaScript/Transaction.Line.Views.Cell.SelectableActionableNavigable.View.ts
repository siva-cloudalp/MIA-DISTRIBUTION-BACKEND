...
const TransactionLineViewsCellSelectableActionableNavigableView: any = BackboneView.extend({
    ...
    getContext: function() {
		...
		if(this.options.invoiceDates && this.options.invoiceDates.length > 0){
			
			var invoiceObj:any = _.findWhere(this.options.invoiceDates,{'item':itemId.toString()});
			
			disableReturns = _.has(invoiceObj,'disableReturns') ? invoiceObj.disableReturns: false;
		}
		/*if(item.get('itemtype') == "Kit"){
			disableReturns = true;
		}*/
