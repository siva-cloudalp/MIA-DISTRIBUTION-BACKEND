...
export = WizardModule.extend({
    ...
    events: {
		...
		'change [name="is_drop_ship"]': 'setDropShip',
		'change [name="is_partial_shipment"]': 'setPartialShipment'
    },
    // @property {Array} errors
    errors: ['ERR_CHK_SELECT_SHIPPING_METHOD', 'ERR_WS_INVALID_SHIPPING_METHOD'],

    shippingMethodsCache: {},
	setDropShip: function(e){
		
		var isDropShip = jQuery(e.currentTarget).val();
		
		this.model.get('options').custbody_is_this_a_dropship = isDropShip;
		this.model.save();
	},
	setPartialShipment: function(e){
		
		this.model.get('options').custbody_is_partial_shipment = jQuery(e.currentTarget).val();
		this.model.save();
	},
    // @method initialize
    initialize: function() {
    ...
    render: function() {
        if (this.state === 'present') {
			...
			var preffered_shipmethod = this.model.get('preffered_shipmethod') || '';
			
			if(preffered_shipmethod && preffered_shipmethod != shipmethod){
				this.model.set('shipmethod',preffered_shipmethod);
				this.model.save();
				return;
			}
            this._render();
    ...
    isValid: function isValid(): any {
		...
		if(!this.$('[name="is_drop_ship"]').val()){
			valid_promise.reject({
				errorCode: 'ERR_CHK_SELECT_SHIPPING_METHOD',
				errorMessage: Utils.translate('Please select is this a dropship?')
			});
		}
		else{
    ...
    getContext: function getContext() {
        ...
        return {
			...
			isDropShipYes: (this.model.get('options').custbody_is_this_a_dropship == '1'),
			isDropShipNo: (this.model.get('options').custbody_is_this_a_dropship == '2'),
			isPartialShipment: (this.model.get('options').custbody_is_partial_shipment == 'T')
        };
