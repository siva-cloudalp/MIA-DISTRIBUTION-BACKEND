...
import FacetsSideBar = require('./FacetsSideBar.View'); 
...
export = BackboneView.extend({
    ...
    events: {
		...
		'click [data-navigate]': 'navigateSections'
    },
    ...
    initialize: function(options) {
        ...
        this.itemsDisplayOptions = Utils.deepCopy(
    ...
    getPath: function() {
        ...
        const canonical = base_url + '/' + Backbone.history.fragment;
    ...
    getTitle: function() {
        ...
        if (facets && facets.length) {
            ...
            title =
                this.application.getConfig('searchTitlePrefix', '') +
                buffer.join('') +
        ...
        if (this.model.get('category')) {
            ...
            _.each(list, function(bread: any) {
                breadcrumb.push({
                    text: bread.name,
                    href: bread.fullurl,
                    additionalFields: CategoriesUtils.getAdditionalFields(
    ...
    getContext: function() {
		...
		var catFields:any = this.model.get('category')
		...
		if(catFields && catFields.length){
			...
			if(catFields.video){
				videoUrl = catFields.video;
			}
			else if(catFields2.video){
				videoUrl = catFields2.video;
			}
			
			if(catFields.rightToVideo){
				videoImage = catFields.rightToVideo;
			}
			else if(catFields2.rightToVideo){
				videoImage = catFields2.rightToVideo;
			}
			var item;
        ...
        return {
			...
			featuredItem: !_.isEmpty(featuredItem)?featuredItem: false,
			videoUrl: videoUrl,
