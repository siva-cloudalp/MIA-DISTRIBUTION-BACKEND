...
const HomeView: any = BackboneView.extend({
    ...
    events: {
		...
		'click [name="brand_filters"]': 'filterBrands'
    },
	...
	filterBrands: function(e){
		...
		var filter = jQuery(e.currentTarget).attr('value');
		
		jQuery('.brand-images a').each(function(){
			
			var allTypes = jQuery(this).attr('data-type');
			...
			if(filter == 'all'){
				...
				var allTypesArr = allTypes.split(',');
				
				if(allTypesArr.indexOf(filter) == -1){
					jQuery(this).hide();
				}
				else{
					jQuery(this).fadeIn('slow');
	...
	beforeShowContent: function(){
		...
		var self = this;
		var promise = new BackboneModel().fetch({url: Utils.getAbsoluteUrl('services/LandingPages.Service.ss'),data:{isHome:true}}).then(function(result){
    ...
    getContext: function() {
        ...
        return {
			...
			types: (this.vendorsData && this.vendorsData.types) || []
            // @class Home.View
        };
    }
});
