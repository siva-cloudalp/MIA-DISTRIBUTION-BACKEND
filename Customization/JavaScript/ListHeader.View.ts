...
const ListHeader: any = BackboneView.extend({
    ...
    events: {
		...
		'change [name="filter_by_po"]': 'poHandler',
		'click [name="filter_by_po_reset"]': 'clearPO'
    },
	clearPO: function(e){
		e.stopPropagation();
    ...
    getContext: function() {
        ...
        return {
            ...
            sorts: this.sorts,
			showSearchPO: this.showSearchPO,
			searchPO: this.selectedPO,
    ...
    setSelecteds: function() {
		...
		this.selectedPO = url_options.PO || '';
    ...
    updateUrl: function() {
		...
		url =
            (!this.selectedPO)
                ? UrlHelper.removeUrlParameter(url, 'PO')
                : UrlHelper.setUrlParameter(url, 'PO', this.selectedPO);
