...
import AddressSelectView = require('./Address.Select.View');
// @class OrderWizard.Module.Address @extend Wizard.Module
const OrderWizardModuleAddress: any = WizardModule.extend({
    ...
    events: {
		...
		'change [name="address_select_v1"]': 'selectAddressV1',
    ...
    initialize: function() {
        ...
        this.model.on('address_added', this.render, this);
    ...
    evaluateDisablingSameAsCheckBox: function(re_render_is_change: boolean) {
        // The other manage address is not selected AND the other temp address is not created
        const old_value = this.disableSameAsCheckBox;
    ...
    prepopulateAddressModel: function() {
        if (this.wizard.isAutoPopulateEnabled && this.wizard.isAutoPopulateEnabled()) {
            this.address.set(
                'fullname',
                this.wizard.options.profile.get('firstname') +
                    ' ' +
                    this.wizard.options.profile.get('lastname')
            );
        }
	...
	childViews: {
		...
		'Address.Select': function() {
            return new AddressSelectView({
                collection: this.getAddressesToShow(),
				addressListOptions: this.getAddressListOptions()
            });
        }
    },
